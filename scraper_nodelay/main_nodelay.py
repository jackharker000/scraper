"""
CLI entry point for the PapaCambridge past-paper scraper (no-delay version).

Usage examples:

    # Dry-run: show what would be downloaded without writing any files
    python main_nodelay.py --dry-run

    # Download a single subject to ./test_output
    python main_nodelay.py --subject igcse-biology-0610 --output test_output

    # Download everything (IGCSE + AS & A Level)
    python main_nodelay.py --output output

    # Download only A Level papers, 10 parallel workers
    python main_nodelay.py --no-igcse --workers 10 --output output
"""
...
import scraper_nodelay as scraper
